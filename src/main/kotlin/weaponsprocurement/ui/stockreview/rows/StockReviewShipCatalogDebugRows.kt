...
object StockReviewShipCatalogDebugRows {
    ...
    fun build(): List<WimGuiListRow<StockReviewAction>> {
        ...
        if (candidates.isEmpty()) {
            rows.add(StockReviewListRow.fromSpec(StockReviewRowSpecs.empty("No theoretical ship candidates are currently available.")))
        } else {
        ...
        if (observedOnly.isEmpty()) {
            rows.add(StockReviewListRow.fromSpec(StockReviewRowSpecs.empty("No observed-only ships in current market cargo.")))
        } else {
