...
object StockReviewShipFilters {
    ...
    fun matches(record: StockReviewShipRecord, state: StockReviewState): Boolean {
        ...
        val slots = visibleWeaponSlots(record)
        if (!matchesMin(countBySize(slots, "SMALL"), state.getShipFilterInt(StockReviewShipFilterField.MIN_SMALL_MOUNTS))) {
            return false
        }
        if (!matchesMin(countBySize(slots, "MEDIUM"), state.getShipFilterInt(StockReviewShipFilterField.MIN_MEDIUM_MOUNTS))) {
            return false
        }
        if (!matchesMin(countBySize(slots, "LARGE"), state.getShipFilterInt(StockReviewShipFilterField.MIN_LARGE_MOUNTS))) {
            return false
        }
        if (!matchesMin(countByType(slots, "ENERGY"), state.getShipFilterInt(StockReviewShipFilterField.MIN_ENERGY_MOUNTS))) {
            return false
        }
        if (!matchesMin(countByType(slots, "BALLISTIC"), state.getShipFilterInt(StockReviewShipFilterField.MIN_BALLISTIC_MOUNTS))) {
            return false
        }
    ...
    private fun ordnancePoints(record: StockReviewShipRecord): Int =
        record.member.hullSpec.getOrdnancePoints(Global.getSector()?.playerStats)
    ...
    private fun countBySize(slots: List<WeaponSlotAPI>, size: String): Int =
        slots.count { it.slotSize.name == size }

    private fun countByType(slots: List<WeaponSlotAPI>, type: String): Int =
        slots.count { it.weaponType.name == type }
