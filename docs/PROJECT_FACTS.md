...
- Commit: `a06fe17ad2a1fc3faab12ceeda106a35d0f310c4`
- Build/check status: GUI style validation, compatibility surfaces, Kotlin migration, full build with `-ShatterLibDir` checkout override, repo jar classes, doc links, template hygiene, deploy status, and `git diff --check` passed. Trade rollback, source semantics, Fixer persistence, ship trading, pure logic, config, and runtime evidence validators were not rerun for this bounded tooltip-renderer ownership change.
