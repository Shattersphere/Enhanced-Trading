...
| Runtime evidence contracts | `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\validate-runtime-evidence-contracts.ps1` | Changing runtime evidence helper, rollback/ship diagnostic analyzers, or required evidence gates | Pass/fail by skip/fail-closed/log-fixture contracts | low |
...
- `tools/validate-runtime-evidence-contracts.ps1` proves the evidence helper's skip, fail-closed, and log-fixture analyzer wiring; it does not create runtime evidence.
