...
- `tools/validate-trade-rollback-contracts.ps1` is static contract coverage for rollback journal order, debug failure steps, diagnostic fields, quote-source guards, and credit/cargo guards; it does not prove rollback succeeds in a live campaign.
...
- `tools/validate-ship-trading-contracts.ps1` is static contract coverage for local-only exact-member ship snapshots, pending ship trades, buy/sell fleet mutation order, mutation failure guards, and remote ship-trading gates; it does not prove in-game ship buy/sell behavior.
