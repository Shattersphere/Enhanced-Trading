...
| GUI button style | `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\validate-gui-button-style.ps1` | GUI/button rendering, tooltip owner, or ship-grid layout changes | Pass/fail plus guarded owner/layout surface | low |
...
| Assertion helper contracts | `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\validate-validation-assertions.ps1` | Changing `tools/lib/Validation.Assertions.ps1` or shared validator output behavior | Pass/fail by helper self-checks | low |
...
| Runtime evidence contracts | `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\validate-runtime-evidence-contracts.ps1` | Changing runtime evidence helper, rollback/ship diagnostic analyzers, or required evidence gates | Pass/fail by skip/fail-closed/log-fixture/schema contracts | low |
| Deploy status | `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\deploy-live-mod.ps1 -Status` | Runtime deploy troubleshooting | Queue/lock/staging status, current file/dependency blocker, and Shatter Lib runtime API state | low; requires Starsector path |
...
- `tools/validate-ship-trading-contracts.ps1` is static contract coverage for local-only exact-member ship snapshots, pending ship trades, buy/sell fleet mutation order, mutation failure guards, post-commit transaction-report isolation, and remote ship-trading gates; it does not prove in-game ship buy/sell behavior.
- `tools/validate-runtime-evidence-contracts.ps1` proves evidence-helper skip/fail-closed behavior, analyzer wiring, missing/malformed/typed rollback-record rejection, malformed ship-summary rejection, and required expected-hull checks; it does not create runtime evidence.
