...
`docs/PROJECT_FACTS.md` declares which shared libraries are authorized and whether this mod currently consumes them at build/runtime. If a library is not declared there and no explicit user task authorizes the named project, report the blocker; do not search sibling or parent directories opportunistically.
...
If the gate is not satisfied, do not edit Shatter Lib. Write a Shatter Lib task packet instead, including the consumer need, why existing public APIs are insufficient, candidate generic API shape, and validation expectations.
...
- Final reports must name both repos' validation, commit hash, branch, push status, and unverified integration behavior when both repos were touched.

## External Dependency Rule

LazyLib and LunaLib are read-only dependency libraries for this workflow. Inspect their public docs/API only when this repo actually uses or declares them. Do not edit, fork, patch, vendor, or reconfigure those libraries unless the user gives separate explicit instructions.
