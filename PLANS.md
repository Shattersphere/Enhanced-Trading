...
- Recent pushed modernization baseline: `d11117c` names pending trade execution phases in `StockReviewTradePlanner` and adds source-semantics validator coverage for sell/source-buy/generic-buy ordering. Earlier source commits centralized stock-review state reset changes and action construction, split tooltip owners, moved ship hull filtering into its filter owner, extracted ship tooltip stat rows, sourced fighter LPC Advanced Info stats, added pure logic contract validation, and extracted purchase rollback journaling. Source/static checks passed for that work, but in-game rollback fault validation plus tooltip and ship UI acceptance were not run.
