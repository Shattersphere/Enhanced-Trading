...
- Recent pushed modernization baseline: `b85c9a4` makes developer-only stock-review debug modes fail closed from input/render paths when the Luna debug UI setting is disabled. Earlier source commits guarded ship-grid layout validation, extracted normal Shatter Lib item-tooltip creation into `StockReviewShatterItemTooltipFactory`, hardened Fixer/ship lifecycle gates, centralized submarket OPEN/SNEAK trade-mode policy, aligned remote-source sell quotes with legal-only execution, documented the legacy `includeCurrentMarketStorage` key as all-accessible storage behavior, centralized Fixer reference-value normalization, shared Fixer source selection, made stock override precedence explicit, named pending trade phases, centralized stock-review state reset changes and action construction, split tooltip owners, moved ship hull filtering into its filter owner, extracted ship tooltip stat rows, sourced fighter LPC Advanced Info stats, added pure logic contract validation, and extracted purchase rollback journaling. Source/static checks passed for that work, but in-game rollback fault validation plus tooltip, ship UI, debug UI, and black-market submarket behavior acceptance were not run.
