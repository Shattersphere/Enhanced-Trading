...
- Recent pushed modernization baseline: `5e75045` makes `StockReviewConfig` override precedence explicit by loading legacy `perWeapon` before current `perItem`, with config and compatibility validators guarding that order. Earlier source commits named pending trade execution phases, centralized stock-review state reset changes and action construction, split tooltip owners, moved ship hull filtering into its filter owner, extracted ship tooltip stat rows, sourced fighter LPC Advanced Info stats, added pure logic contract validation, and extracted purchase rollback journaling. Source/static checks passed for that work, but in-game rollback fault validation plus tooltip and ship UI acceptance were not run.
