...
- Recent pushed modernization baseline: `a04fdab` shares Fixer reference-source selection between persistent and live observation, preserving purchasable-source filtering and lowest-price/source-name tie-breaking with Fixer persistence validator coverage. Earlier source commits made stock override precedence explicit, named pending trade phases, centralized stock-review state reset changes and action construction, split tooltip owners, moved ship hull filtering into its filter owner, extracted ship tooltip stat rows, sourced fighter LPC Advanced Info stats, added pure logic contract validation, and extracted purchase rollback journaling. Source/static checks passed for that work, but in-game rollback fault validation plus tooltip and ship UI acceptance were not run.
