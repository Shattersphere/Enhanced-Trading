...
- Recent pushed modernization baseline: `a06fe17` extracts custom weapon fallback tooltip rendering from `StockReviewItemTooltip` into `StockReviewWeaponTooltipRenderer` and updates the GUI validator to follow that owner. Earlier source commits split stock tooltip models, icon-panel rendering, wing tooltip rendering/layout construction, weapon tooltip row building, item tooltip context construction, weapon tooltip icon-grid rendering, and weapon tooltip text rendering, then sourced fighter LPC Advanced Info stats and added pure logic contract validation. Source/static checks passed for that work, but in-game tooltip acceptance was not run.
