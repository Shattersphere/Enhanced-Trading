...
- Recent pushed modernization baseline: `5811cdc` centralizes submarket OPEN/SNEAK trade-mode policy in `StockSubmarketTradeModes` so eligibility probing and item/ship transaction reporting cannot drift. Earlier source commits acted on the static review findings by aligning remote-source sell quotes with legal-only execution, documenting the legacy `includeCurrentMarketStorage` key as all-accessible storage behavior, using OPEN/SNEAK trade-mode probing for submarket eligibility, centralizing Fixer reference-value normalization, sharing Fixer source selection, making stock override precedence explicit, naming pending trade phases, centralizing stock-review state reset changes and action construction, splitting tooltip owners, moving ship hull filtering into its filter owner, extracting ship tooltip stat rows, sourcing fighter LPC Advanced Info stats, adding pure logic contract validation, and extracting purchase rollback journaling. Source/static checks passed for that work, but in-game rollback fault validation plus tooltip, ship UI, and black-market submarket behavior acceptance were not run.
