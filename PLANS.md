...
- Recent pushed modernization baseline: `b876bc6` adds no-framework pure logic contract validation for item keys, sort aliases, and `TradeMoney` guards. Earlier source commits split stock tooltip models, icon-panel rendering, wing tooltip rendering/layout construction, weapon tooltip row building, item tooltip context construction, weapon tooltip icon-grid rendering, and weapon tooltip text rendering, then sourced fighter LPC Advanced Info stats from real fighter hull data. Source/static checks passed for that work, but in-game tooltip acceptance was not run.
