...
- Recent pushed modernization baseline: `0157fad` centralizes `StockReviewAction` construction behind named payload parameters, reducing positional-null drift in GUI action routing while preserving factory names and action payload semantics. Earlier source commits split tooltip owners, moved ship hull filtering into its filter owner, extracted ship tooltip stat rows, sourced fighter LPC Advanced Info stats, added pure logic contract validation, and extracted purchase rollback journaling. Source/static checks passed for that work, but in-game rollback fault validation plus tooltip and ship UI acceptance were not run.
