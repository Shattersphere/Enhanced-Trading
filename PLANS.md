...
- Recent pushed modernization baseline: `8d9ed91` acts on the second static review packet by publishing Luna settings before Fixer updater registration, decoupling ship black-market state from hidden item source mode, opening for local ship-only opportunities, and rechecking ship sell eligibility at confirmation. Earlier source commits centralized submarket OPEN/SNEAK trade-mode policy, aligned remote-source sell quotes with legal-only execution, documented the legacy `includeCurrentMarketStorage` key as all-accessible storage behavior, centralized Fixer reference-value normalization, shared Fixer source selection, made stock override precedence explicit, named pending trade phases, centralized stock-review state reset changes and action construction, split tooltip owners, moved ship hull filtering into its filter owner, extracted ship tooltip stat rows, sourced fighter LPC Advanced Info stats, added pure logic contract validation, and extracted purchase rollback journaling. Source/static checks passed for that work, but in-game rollback fault validation plus tooltip, ship UI, and black-market submarket behavior acceptance were not run.
