...
- Recent pushed modernization baseline: `34f608c` extracts purchase rollback bookkeeping from `StockPurchaseExecutor` into `StockPurchaseRollbackJournal`, leaving the executor focused on mutation sequencing, transaction reports, and rollback diagnostics. Earlier source commits split tooltip owners, moved ship hull filtering into its filter owner, extracted ship tooltip stat rows, sourced fighter LPC Advanced Info stats, and added pure logic contract validation. Source/static checks passed for that work, but in-game rollback fault validation plus tooltip and ship UI acceptance were not run.
