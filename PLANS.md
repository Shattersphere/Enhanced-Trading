...
- Recent pushed modernization baseline: `6661f91` moves ship hull-class filtering into `StockReviewShipFilters`, leaving `StockReviewShipGridRenderer` focused on layout/rendering and updating ship contract validation to guard that owner boundary. Earlier source commits split tooltip owners, sourced fighter LPC Advanced Info stats, and added pure logic contract validation. Source/static checks passed for that work, but in-game tooltip and ship UI acceptance were not run.
