...
- Recent pushed modernization baseline: `8252e0f` centralizes list-scroll reset and content-revision bumps for `StockReviewState` filter/search/trade-kind changes, reducing GUI state drift while preserving which actions reset to the top. Earlier source commits centralized `StockReviewAction` construction, split tooltip owners, moved ship hull filtering into its filter owner, extracted ship tooltip stat rows, sourced fighter LPC Advanced Info stats, added pure logic contract validation, and extracted purchase rollback journaling. Source/static checks passed for that work, but in-game rollback fault validation plus tooltip and ship UI acceptance were not run.
